...
int ler_arq(char **str, FILE *arq, char stop_char){//extrai todo o conteudo do arquivo p/ uma string
    *str = NULL;//Eh a string que recebe o conteudo do arquivo
    int tam=0;//Eh o tamanho da string, aumenta dinamicamente
    for (char c = getc(arq); !((c == stop_char)||(c == EOF)); c = getc(arq)){//lendo arquivo char p/ char
        *str = realloc(*str, (tam+1) * sizeof(char));
        ...
        (*str)[tam]=c;//inserindo o char na string
        tam++;
    }
    if(*str==NULL)
        return -1;//erro de execucao: não foi possivel ler
    (*str)[tam]='\0';//indicando o fim da string
    return 1;
}
